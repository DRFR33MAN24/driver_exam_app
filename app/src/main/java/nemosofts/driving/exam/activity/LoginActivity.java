...
import nemosofts.driving.exam.ProCompatActivityNormal;
import nemosofts.driving.exam.view.SmoothCheckBox;
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //  et_login_email = findViewById(R.id.et_login_email);
        ...
        if(sharedPref.getIsRemember()) {
            et_login_id.setText(sharedPref.getIdNumber());
...
//        }
        ...
        if (TextUtils.isEmpty(num_id)) {
            et_login_id.setError(getString(R.string.error_invalid_id_num));
            focusView = et_login_id;
            cancel = true;
        }
    ...
    private void loadLogin() {
        if (helper.isNetworkAvailable()) {
                ...
                public void onEnd(String success, String loginSuccess, String message, String user_id, String user_name, String user_gender, String user_phone,String profile_img) {
                    ...
                    if (success.equals("1")) {
                        if (loginSuccess.equals("1")) {
                            sharedPref.setLoginDetails(user_id, user_name, user_phone,et_login_id.getText().toString(),"", user_gender, profile_img, "", remember_me.isChecked(), et_login_password.getText().toString(), Callback.LOGIN_TYPE_NORMAL);
                            sharedPref.setIsFirst(false);
            ...
            }, helper.callAPI(Callback.METHOD_LOGIN, 0,"","","","","","",et_login_id.getText().toString(),"","","",et_login_password.getText().toString(),"", Callback.LOGIN_TYPE_NORMAL,null));
            loadLogin.execute();
        } else {
            Toast.makeText(LoginActivity.this, getString(R.string.error_internet_not_connected), Toast.LENGTH_SHORT).show();
        }
    ...
    private void loadLoginSocial(final String name, String email, final String authId) {
        if (helper.isNetworkAvailable()) {
            LoadRegister loadRegister = new LoadRegister(new SocialLoginListener() {
                ...
                @Override
                public void onEnd(String success, String registerSuccess, String message, String user_id, String user_name, String email,String id_num, String auth_id) {
                    progressDialog.dismiss();
                    if (success.equals("1")) {
                        if (registerSuccess.equals("1")) {
                            sharedPref.setLoginDetails(user_id, user_name, "",id_num, email, "", "", authId, remember_me.isChecked(), "", Callback.LOGIN_TYPE_GOOGLE);
                            sharedPref.setIsFirst(false);
                            sharedPref.setIsLogged(true);
                            sharedPref.setIsAutoLogin(true);
